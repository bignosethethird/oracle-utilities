...
begin
  ...
  -- Test statements  
  i:=utl.scheduler.add_task(null,    -- Type of task: DURABLE (default), VOLATILE, RESTARTABLE
  ...
  v_exp:=utl.scheduler.get_task_explanation(i);
  if(length(v_exp)=0)then
    v_retcode:=-1;
  else
    for j in 1..length(v_exp)/60 loop
      dbms_output.put_line(substr(v_exp,((j-1)*60)+1,60));      
    end loop;
  end if;                         
